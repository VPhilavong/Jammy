...
export default function TopTracksPage(): React.JSX.Element {
  ...
  const SkeletonCard = (): React.JSX.Element => (
    <div className="bg-sage/10 backdrop-blur-sm rounded-lg p-4 border border-sage/20">
      <div className="relative mb-4">
        <div className="aspect-square rounded-md bg-sage/20 animate-pulse" />
      </div>
      <div className="space-y-3">
        <div className="space-y-2">
          <div className="h-4 bg-sage/20 rounded animate-pulse" />
          <div className="h-3 bg-sage/15 rounded animate-pulse w-3/4" />
        </div>

        {/* Artist avatars skeleton */}
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="h-3 w-12 bg-sage/15 rounded animate-pulse" />
            <div className="flex -space-x-1">
              <div className="w-8 h-8 rounded-full bg-sage/20 animate-pulse" />
              <div className="w-8 h-8 rounded-full bg-sage/20 animate-pulse" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <div className="h-2 bg-sage/15 rounded animate-pulse" />
            <div className="w-full bg-sage/20 rounded-full h-1" />
          </div>
          <div className="text-right space-y-1">
            <div className="h-2 bg-sage/15 rounded animate-pulse w-12 ml-auto" />
            <div className="h-3 bg-sage/20 rounded animate-pulse w-16 ml-auto" />
          </div>
        </div>
  ...
  const SkeletonAvatar = (): React.JSX.Element => (
    <div className="w-8 h-8 rounded-full bg-sage/20 animate-pulse flex-shrink-0" />
  )
  ...
  const ArtistAvatar = ({ artist }: { artist: Artist }): React.JSX.Element => {
    ...
    return (
      <div
        className="w-8 h-8 rounded-full border border-sage/30 overflow-hidden bg-sage/20 flex-shrink-0 transition-all duration-300"
        title={artist.name}
      >
        {artistImage ? (
          <Image
          ...
          <div className="w-full h-full flex items-center justify-center">
            <span className="text-dark-green text-xs font-medium">{artist.name.charAt(0).toUpperCase()}</span>
          </div>
        )}
      </div>
    )
  ...
  const TrackCard = ({ track, index }: { track: Track; index: number }): React.JSX.Element => (
    ...
    >
      ...
      <div className="absolute -top-2 -right-2 z-10">
        <div className="bg-gradient-to-r from-copper to-gold text-cream rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold shadow-lg border border-gold/20">
          #{index + 1}
        </div>
      </div>
      ...
      {/* Album Art */}
      <div className="relative mb-4">
        <div className="aspect-square rounded-md overflow-hidden bg-sage/20 shadow-lg ring-1 ring-sage/30">
          {track.album.images?.[0]?.url ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-gold to-copper flex items-center justify-center">
      ...
      <div className="space-y-3">
        <div>
          <h3 className="text-dark-green font-semibold text-base leading-tight mb-1 line-clamp-2 group-hover:text-sage transition-colors">
            {track.name}
          </h3>
          <p className="text-sage text-sm font-normal">{track.artists.map((artist) => artist.name).join(", ")}</p>
        </div>

        {/* Artist Avatars */}
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-sage text-xs font-medium uppercase tracking-wide">Artists</span>
            <div className="flex -space-x-1">
              {track.artists.slice(0, 3).map((artist, artistIndex) => (
                <ArtistAvatar key={artistIndex} artist={artist} />
              ))}
              {track.artists.length > 3 && (
                <div className="w-8 h-8 rounded-full border border-sage/30 bg-sage/30 flex items-center justify-center text-xs text-cream font-medium">
                  +{track.artists.length - 3}
                </div>
              )}
            </div>
          ...
          <div className="flex items-center space-x-1">
            {track.external_urls?.spotify && (
              <a
                ...
                className="text-sage hover:text-green-400 transition-colors p-1"
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-right">
            <p className="text-xs text-sage mb-1">Duration</p>
            <p className="text-dark-green font-mono text-sm font-medium">{formatDuration(track.duration_ms)}</p>
          </div>
        </div>
        ...
        {track.genres && track.genres.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-1">
            {track.genres.slice(0, 2).map((genre, genreIndex) => (
              <span
                key={genreIndex}
                className="px-2 py-1 bg-gradient-to-r from-gold/80 to-copper/80 text-cream rounded-full text-xs font-medium backdrop-blur-sm border border-gold/20"
              >
                {genre}
              </span>
            ))}
            {track.genres.length > 2 && (
              <span className="px-2 py-1 bg-sage/30 text-cream rounded-full text-xs backdrop-blur-sm">
                +{track.genres.length - 2}
              </span>
            )}
          </div>
      ...
      <div className="relative max-w-7xl mx-auto px-8 py-12">
        <div className="text-center mb-8">
          <p className="text-dark-green text-lg font-medium">
            Showing your top <span className="font-bold text-sage">{tracks.length}</span> tracks
          </p>
        </div>
